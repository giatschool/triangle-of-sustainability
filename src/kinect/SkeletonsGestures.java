...
public class SkeletonsGestures {
	...
	/*
	 * stores gesture sequences for each user, and looks for more complex
	 * gestures (actually only right hand waving at present)
	 */
	...
	private boolean cameraPose = false;
	...
	/*
	 * decide which gestures have just started or just finished, and notify the
	 * watcher.
	 */
	{
		...
		/*
		 * leanLeft(userID, skel); leanRight(userID, skel); leanFwd(userID,
		 * skel); leanBack(userID, skel);
		 */
		...
		/*
		 * leftHandFwd(userID, skel); leftHandOut(userID, skel);
		 * lefttHandIn(userID, skel); leftHandDown(userID, skel);
		 */
		// leftHandUp(userID, skel);
	} // end of checkGests()

	private void camera(int userID,
		...
		if ((leftHandPt == null) || (rightHandPt == null) || (headPt == null))
			return;

		// float xDiff = rightHandPt.getX() - leftHandPt.getX();
		float distLeftToHead = distApart(leftHandPt, headPt);
		float distRightToHead = distApart(rightHandPt, headPt);
		// System.out.println(distLeftToHead+" "+distRightToHead);
		if (distLeftToHead < 60 && distRightToHead < 60) { // near
			if (!cameraPose) {
				watcher.pose(userID, GestureName.CAMERA, true); // started
				cameraPose = true;
			}
		} else { // not near
			if (cameraPose) {
				watcher.pose(userID, GestureName.CAMERA, false); // stopped
				cameraPose = false;
			}
		}
	...
	// is the user's right hand at head level or above?
	{
		...
		if (rightHandPt.getY() <= headPt.getY()) { // above
			if (!isRightHandUp) {
				...
				gestSeqs.addUserGest(userID, GestureName.RH_UP); // add to
																	// gesture
																	// sequence
				isRightHandUp = true;
			}
	...
	// is the user's right hand forward of his right shoulder?
	{
		...
		if (zDiff < -1 * (armLength * 0.95f)) { // is forward
			...
			if (!isRightHandFwd) {
				...
				gestSeqs.addUserGest(userID, GestureName.RH_FWD); // add to
																	// gesture
																	// sequence
				isRightHandFwd = true;
			}
	...
	// is the user's right hand out to the right of the his right elbow?
	{
		...
		if (xDiff > (lowerArmLength * 0.6f)) { // out to the right
			if (!isRightHandOut) {
				...
				gestSeqs.addUserGest(userID, GestureName.RH_OUT); // add to
																	// gesture
																	// sequence
				isRightHandOut = true;
			}
	...
	// is the user's right hand inside (left) of his right elbow?
	{
		...
		if (xDiff < -1 * (lowerArmLength * 0.6f)) { // inside
			if (!isRightHandIn) {
				...
				gestSeqs.addUserGest(userID, GestureName.RH_IN); // add to
																	// gesture
																	// sequence
				isRightHandIn = true;
			}
	...
	// is the user's right hand at hip level or below?
	{
		...
		if (rightHandPt.getY() >= hipPt.getY()) { // below
			if (!isRightHandDown) {
				...
				gestSeqs.addUserGest(userID, GestureName.RH_DOWN); // add to
																	// gesture
																	// sequence
