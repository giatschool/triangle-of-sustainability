...
public class AmazonLanduseLayer extends RenderableLayer {
	...
	public AmazonLanduseLayer(ArrayList<MunicipalityDataItem> muniData,
		...
		for (int i = 0; i < mesoRegions.size(); i++) {
			for (MicroRegion micro : mesoRegions.get(i).getMicoRegions()) {
				...
				for (MunicipalityDataItem muniItem : micro.getMuniItems()) {
					if (year == "2003") {
						totalAcumDeforestation += muniItem.getTotalAcum2003()
								* muniItem.getArea();
						totalArea = muniItem.getArea();
					}
					if (year == "2004") {
						totalAcumDeforestation += muniItem.getTotalAcum2004()
								* muniItem.getArea();
						totalArea = muniItem.getArea();
					}
					if (year == "2005") {
						totalAcumDeforestation += muniItem.getTotalAcum2005()
								* muniItem.getArea();
						totalArea = muniItem.getArea();
					}
					if (year == "2006") {
						totalAcumDeforestation += muniItem.getTotalAcum2006()
								* muniItem.getArea();
						totalArea = muniItem.getArea();
					}
					...
					for (int k = 0; k < muniItem.getOverlapArea().size(); k++) {
						if (year == "2002") {
							totalPasturePercentage += muniItem.getPast06().get(
									k)
									* muniItem.getOverlapArea().get(k);
							totalAgriculturePercentage += (muniItem
									.getTempAgr06().get(k) + muniItem
									.getPermAgr06().get(k))
									* muniItem.getOverlapArea().get(k);
							totalAcumDeforestation += muniItem.getAcum2002()
									.get(k) * muniItem.getOverlapArea().get(k);
						...
						if (year == "2007") {
							...
							totalAgriculturePercentage += (muniItem
									...
									.getPermAgr06().get(k))
									* muniItem.getOverlapArea().get(k);
							totalAcumDeforestation += muniItem.getAcum2007()
									.get(k) * muniItem.getOverlapArea().get(k);
							totalArea += muniItem.getOverlapArea().get(k);
						}
						if (year == "2008") {
							...
							totalAgriculturePercentage += (muniItem
									...
									.getPermAgr06().get(k))
									* muniItem.getOverlapArea().get(k);
							totalAcumDeforestation += muniItem.getAcum2008()
									.get(k) * muniItem.getOverlapArea().get(k);
							totalArea += muniItem.getOverlapArea().get(k);
						}
					}
				...
				totalAgriculturePercentage = totalAgriculturePercentage
						/ totalArea;
				...
				for (String str : borderStringArray) {
					String[] splitStr = str.split(",");
					borderPositions
							.add(Position.fromDegrees(
									Double.parseDouble(splitStr[1]),
									Double.parseDouble(splitStr[0]),
									(totalPasturePercentage + totalAgriculturePercentage)
											* 1000000 * totalAcumDeforestation));
				}
				...
				// tempBorders.add(new
				// Position(borderPositions2.get(k).latitude,
				...
				pgon2.setAltitudes(
						(totalPasturePercentage + totalAgriculturePercentage)
								* 1000000 * totalAcumDeforestation + 1,
						1000000 * totalAcumDeforestation);
				...
				// //
				// pgon3.setAltitudes(totalAgriculturePercentage/totalArea
				// *1000000 *totalAcumDeforestation +
				// totalPasturePercentage/totalArea *1000000
				// *totalAcumDeforestation+1, 1000000 *totalAcumDeforestation);
	...
	public void drawArc() {
		...
		addRenderable(exportAnnotation(new Position(posis.next(), 42000.0),
				"Großbritannien: " + unitedKingdomExport));
	...
	public AnnotationLayer generalInformationLayer() {
		...
		if (year == "2004") {
			factAnnotation
					.setText("Die ersten Rinder kamen 1493 mit Columbus nach Südamerika.");
			statAnnotation.setText("Pará \n Fläche: "
					+ f2.format((int) totalArea)
					+ " km²\n Abgeholzte Fläche: ~"
					+ f2.format((int) totalDeforestArea)
