...
public class AmazonPopAcumLayer extends RenderableLayer {
	...
	final static PowerOfTwoPaddedImage badPic = PowerOfTwoPaddedImage
			.fromPath("images/deforestationPic.jpg");

	final static PowerOfTwoPaddedImage trees = PowerOfTwoPaddedImage
			.fromPath("images/bäume.jpg");
	...
	public AmazonPopAcumLayer(ArrayList<MunicipalityDataItem> muniData,
			String year) {
		super();
		this.year = year;
	...
	public AnnotationLayer addAnnotations() {
		...
		ScreenAnnotation sa = new ScreenAnnotation(
				"Abholzung des Regenwaldes und Bevölkerung in Pará, Brasilien",
				new Point(470, 700));
		...
		ScreenAnnotation legendHeader = new ScreenAnnotation(
				"Abholzung des Regenwaldes in %", new Point(930, 270));
		...
		ScreenAnnotation legend2 = new ScreenAnnotation("40-55%", new Point(
				920, 395));
		...
		ScreenAnnotation legend3 = new ScreenAnnotation("25-40%", new Point(
				920, 365));
		...
		ScreenAnnotation legend4 = new ScreenAnnotation("15-25%", new Point(
				920, 335));
		legend4.getAttributes().setDefaults(defaultAttributes2);
		legend4.getAttributes().setImageSource(
				darkYellowPic.getPowerOfTwoImage());
		...
		ScreenAnnotation legend5 = new ScreenAnnotation("5-15%", new Point(920,
				305));
		...
		// legend4.getAttributes().setSize(new Dimension(50, 0));
		legend5.getAttributes().setAdjustWidthToText(AVKey.SIZE_FIXED); // use
																		// strict
																		// dimension
																		// width
																		// - 200
		...
		// legend4.getAttributes().setSize(new Dimension(50, 0));
		legend6.getAttributes().setAdjustWidthToText(AVKey.SIZE_FIXED); // use
																		// strict
																		// dimension
																		// width
																		// - 200
		...
		ScreenAnnotation heightLegend = new ScreenAnnotation(
				"Höhe entspricht der absoluten Bevölkerung", new Point(920, 65));
		...
		// GlobeAnnotation ga = new
		// GlobeAnnotation(muniData.get(maxPos).getName() +
		// ": "+f2.format(maxpop), (Position) pos,
		// Font.decode("Arial-BOLD-12"));
		// ga.getAttributes().setDefaults(defaultAttributes);
		//
		// annotationLayer.addAnnotation(ga);
	...
	public AnnotationLayer generalInformationLayer() {
		...
		for (int j = 0; j < muniData.size(); j++) {
			...
			if (year == "2002") {
				averageAcumDefor += muniData.get(j).getTotalAcum2002()
						* muniData.get(j).getArea();
			}
			if (year == "2003") {
				averageAcumDefor += muniData.get(j).getTotalAcum2003()
						* muniData.get(j).getArea();
			}
			if (year == "2004") {
		...
		if (year == "2002") {
			factAnnotation
					.setText("Pará ist Brasiliens zweit größter Bundesstaat.");
			statAnnotation.setText("Pará \n Bevölkerung: "
					+ f2.format((int) population) + "\n Gesamte Abholzung: "
					+ f.format(averageAcumDefor * 100)
					+ "%\n Bevölkerungsentwicklung: "
					+ f2.format(population - populationLastYear));
		}
		if (year == "2003") {
			factAnnotation
					.setText("Belém, Parás Hauptstadt, ist Brasiliens 11. größte Stadt. ");
			statAnnotation.setText("Pará \n Bevölkerung: "
					+ f2.format((int) population) + "\n Gesamte Abholzung: "
					+ f.format(averageAcumDefor * 100)
					+ "%\n Bevölkerungsentwicklung: "
		...
		if (year == "2004") {
			factAnnotation.setText("In Pará gibt es mehr Rinder ("
					+ f2.format(cattleInventory)
					+ ") als Einwohner in den Niederlanden (~16.000.000).");
			statAnnotation.setText("Pará \n Bevölkerung: "
					+ f2.format((int) population) + "\n Gesamte Abholzung: "
					+ f.format(averageAcumDefor * 100)
					+ "%\n Bevölkerungsentwicklung: "
		...
		if (year == "2005") {
			factAnnotation
					.setText("Belém wurde 1616 von den Portugiesen gegründet.");
			statAnnotation.setText("Pará \n Bevölkerung: "
					+ f2.format((int) population) + "\n Gesamte Abholzung: "
					+ f.format(averageAcumDefor * 100)
					+ "%\n Bevölkerungsentwicklung: "
		...
		if (year == "2006") {
			factAnnotation.setText("In Pará leben "
					+ f2.format(cattleInventory) + " Rinder, das entspricht "
					+ f.format(cattleInventory / population)
		...
		if (year == "2007") {
			factAnnotation
					.setText("Pará gehört zu den ärmeren Bundesstaaten Brasiliens.");
			statAnnotation.setText("Pará \n Bevölkerung: "
					+ f2.format((int) population) + "\n Gesamte Abholzung: "
					+ f.format(averageAcumDefor * 100)
					+ "%\n Bevölkerungsentwicklung: "
		...
		if (year == "2008") {
			factAnnotation
					.setText("Die Bevölkerung Pará's entspricht ungefähr der Bevölkerung der Schweiz auf "
							+ f.format(totalArea / 41285) + " mal der Fläche.");
			statAnnotation.setText("Pará \n Bevölkerung: "
					+ f2.format((int) population) + "\n Gesamte Abholzung: "
					+ f.format(averageAcumDefor * 100)
					+ "%\n Bevölkerungsentwicklung: "
		...
		// if(year=="2005"){
		// ScreenAnnotation statement2005Annotation = new
		...
		// statement2005Annotation.getAttributes().setBackgroundColor(new
		...
		// statement2005Annotation.getAttributes().setSize(new Dimension(300,
		// 190));
		// statement2005Annotation.setAlwaysOnTop(false);
		// statement2005Annotation.getAttributes().setAdjustWidthToText(AVKey.SIZE_FIT_TEXT);
		// // use strict dimension width - 200
		//
		// generalAnnotationLayer.addAnnotation(statement2005Annotation);
		// }
		// if(year=="2006"){
		// ScreenAnnotation statement2006Annotation = new
		// ScreenAnnotation("\"Brasiliens Fleisch Exporte steigen um 14% Jährlich\"",
		// new Point(165, 380));
		// statement2006Annotation.getAttributes().setDefaults(defaultAttributes2);
		// statement2006Annotation.getAttributes().setBackgroundColor(new
		// Color(0.2f, 0.2f, 0.2f, .5f));
		// // backgroundAnnotation.getAttributes().setOpacity(0.5);
		...
		// statement2006Annotation.getAttributes().setSize(new Dimension(300,
		// 190));
		// statement2006Annotation.setAlwaysOnTop(false);
		// statement2006Annotation.getAttributes().setAdjustWidthToText(AVKey.SIZE_FIT_TEXT);
		// // use strict dimension width - 200
		//
