...
public class AmazonDeforestation extends RenderableLayer {
	...
	public AmazonDeforestation(ArrayList<MunicipalityDataItem> muniData,
			String year) {
		super();
		this.year = year;
	...
	public void setAttributes() {
		...
		sideAttributesPlain.setInteriorMaterial(Material.ORANGE);
		sideAttributesPlain.setOutlineOpacity(0.5);
		sideAttributesPlain.setInteriorOpacity(0.5);
		sideAttributesPlain.setOutlineMaterial(Material.ORANGE);
		...
		sideHighlightAttributesPlain = new BasicShapeAttributes(
				sideAttributesPlain);
		sideHighlightAttributesPlain.setOutlineMaterial(Material.ORANGE);
		sideHighlightAttributesPlain.setOutlineOpacity(0.05);

		capAttributesPlain = new BasicShapeAttributes(sideAttributesPlain);
		capAttributesPlain.setInteriorMaterial(Material.ORANGE);
		...
		sideAttributes4.setInteriorMaterial(brown);
		sideAttributes4.setOutlineOpacity(1);
		sideAttributes4.setInteriorOpacity(1);
		sideAttributes4.setOutlineMaterial(brown);
		...
		capAttributes4.setInteriorMaterial(brown);
	...
	public void drawPolygons() {
		...
		for (int j = 0; j < muniData.size(); j++) {
			if (year == "2002") {
				acumDefor = muniData.get(j).getTotalAcum2002();
		...
		for (int i = 0; i < muniData.size(); i++) {
			...
			if (year == "2007") {
				defor = muniData.get(i).getTotalAcum2007();
			}
			if (year == "2008") {
				defor = muniData.get(i).getTotalAcum2008();
			}
			// if(defor > maxDefor){
			// maxDefor = defor;
			// }
			...
			if (year == "2002") {
				acumDefor = muniData.get(i).getTotalAcum2002();
			...
			if (year == "2003") {
				acumDefor = muniData.get(i).getTotalAcum2003();
			}
			if (year == "2004") {
				acumDefor = muniData.get(i).getTotalAcum2004();
			}
			if (year == "2005") {
				acumDefor = muniData.get(i).getTotalAcum2005();
			}
			if (year == "2006") {
				acumDefor = muniData.get(i).getTotalAcum2006();
			}
			if (year == "2007") {
				acumDefor = muniData.get(i).getTotalAcum2007();
			}
			if (year.equals("2008")) {
				acumDefor = muniData.get(i).getTotalAcum2008();
			}
			...
			for (String str : borderStringArray) {
				...
				borderPositions
						.add(Position.fromDegrees(
								...
								acumDefor
										* 1000000
										* (muniData.get(i).getTotalPast06()
												+ muniData.get(i)
														.getTotalPermAgr06() + muniData
												.get(i).getTotalTempAgr06())));
			}
			...
			pgon2.setAltitudes(acumDefor
					* 1000000.0
					* (muniData.get(i).getTotalPast06()
							+ muniData.get(i).getTotalPermAgr06() + muniData
							.get(i).getTotalTempAgr06()), acumDefor * 1000000.0);
			Material brown = new Material(new Color(138, 90, 43));
			pgon2.getAttributes().setMaterial(Material.YELLOW);
			addRenderable(pgon2);

			// pgon3 = new Polygon(borderPositions2);
			//
			// pgon3.setAltitudes(pop * 1000000 *
			// (1-muniData.get(i).getTotalPermAgr06()+muniData.get(i).getTotalTempAgr06()),
			// pop *1000000);
			...
			// pgon.setAttributes(sideAttributesPlain);
			// pgon.setSideHighlightAttributes(sideHighlightAttributesPlain);
			// pgon.setCapAttributes(capAttributesPlain);
	...
	public void drawArc() {
		...
		curtain = new Curtain();
		curtain.setLocations(Arrays.asList(LatLon.fromDegrees(27.0, -112.0),
		...
		posis.next();
		addRenderable(exportAnnotation(new Position(posis.next(), 50000.0),
				"Soja Export nach Europa"));
		addRenderable(route);
		...
		addRenderable(exportAnnotation(route2.getReferencePosition(),
				"Soja Export nach China"));
		// attributesPlain.setInteriorOpacity(1);
	...
	public AnnotationLayer addAnnotations() {
		...
		ScreenAnnotation sa = new ScreenAnnotation(
				"Gesamte- und Jährliche Abholzung des Regenwaldes in Para, Brazilien",
				new Point(900, 1000));
		...
		ScreenAnnotation sa2 = new ScreenAnnotation("" + trees, new Point(1750,
				60));
		...
		ScreenAnnotation legendHeader = new ScreenAnnotation(
				"Jährliche Abholzung des Regenwaldes in % (" + year + ")",
				new Point(1750, 125));
