...
//	static ArrayList<Literal> pop9;
    ...
    private final static PowerOfTwoPaddedImage colorSchemePic =
            PowerOfTwoPaddedImage.fromPath("images/farbverlauf3.jpg");
    
    private final static PowerOfTwoPaddedImage badPic =
            PowerOfTwoPaddedImage.fromPath("images/Amazonie_deforestation.jpg");
    private final static PowerOfTwoPaddedImage trees =
            PowerOfTwoPaddedImage.fromPath("images/bäume2.jpg");
...
//		  double minDefor=2000000;
		  ...
		  for(int j=0;j< muniData.size();j++){
	          		...
	          		if(tempPop > maxpop){
	 //         			System.out.println(muniData.get(j).getName() + " "+ (int)(muniData.get(j).getGdp04()/muniData.get(j).getPop2004()));
	          			maxpop = tempPop;
	          			maxPos = j;
	          		}
	          		if(tempPop < minpop){
        ...
        //  	String defor2008String = muniData.get(i).get(i).toString().replace("^^http://www.w3.org/2001/XMLSchema#integer", "");
          ...
          //	System.out.println(gdp +" "+ pop +" " + gdp/pop);
          	...
          	for(String str: borderStringArray){
          		...
          		borderPositions.add(Position.fromDegrees(Double.parseDouble(splitStr[1]), Double.parseDouble(splitStr[0]),8000* gdp/pop ));
         	...
         	// Material mat = new Material(new Color((int)(255*totalAcumDefor),(255-(int)(255*totalAcumDefor))/2,0));
         	Material mat;
         	 //COLOR SCHEME RED TO YELLOW (works fine)
//       	  mat = new Material(new Color(255, (int)((1-Math.sqrt(Math.sqrt(totalAcumDefor)))*200),0));

         	 if(totalAcumDefor != 0 && Math.abs(Math.log(totalAcumDefor)) < 10){

         	  mat = new Material(new Color(255, Math.abs((int)((Math.log(totalAcumDefor))*255/(10))),0));
         	 }else{
             	mat = new Material(new Color(0, 255,0));
             //	System.out.println(totalAcumDefor);
         	 	}
         //	Material mat = new Material(new Color(255, (int)(Math.log(totalAcumDefor)*255),0));
...
//         	 }
        	...
        	String[] splitStr = muniData.get(i).getCentroid().split(",");	
        	 ...
        	 if(muniData.get(i).getName().equals("SantarÃ©m") || muniData.get(i).getName().contains("Belém") || muniData.get(i).getName().contains("Altamira") || i == maxPos){
        	   GlobeAnnotation ga2 = new GlobeAnnotation(muniData.get(i).getName()+"\nBIP/Person: "+f2.format(gdp*1000/pop*0.400125419)+" R$ ",Position.fromDegrees(Double.parseDouble(splitStr[1]), Double.parseDouble(splitStr[0]),8000* gdp/pop), Font.decode("Arial-BOLD-12"));
         ...
         annotationLayer.addAnnotation(ga2);
          	...
          	addRenderable(pgon);
        	if(i ==maxPos){
       			addPictureLayer(Position.fromDegrees(Double.parseDouble(splitStr[1]), Double.parseDouble(splitStr[0]), 8000* gdp/pop), badPic, 0.5);
       		}
       		else if (i == minPos){
       			addPictureLayer(Position.fromDegrees(Double.parseDouble(splitStr[1]), Double.parseDouble(splitStr[0]), 8000* gdp/pop), trees,0.2);
...
//         System.out.println("min: "+minDefor +" max: "+maxDefor);
	...
	public AnnotationLayer addAnnotations(){
        ...
        ScreenAnnotation sa = new ScreenAnnotation("Bruttoinlandsprodukt und jährliche Abholzungsrate Pará, Brasilien", new Point(470, 700));
     ...
     //   sa.getAttributes().setAdjustWidthToText(AVKey.SIZE_FIXED); // use strict dimension width - 200
        ...
        ScreenAnnotation sa2 = new ScreenAnnotation("Jahr "+year, new Point(125, 550));
 ...
 //       defaultAttributes2.setBackgroundColor(new Color(0f, 0f, 0f, .5f));
        ...
        ScreenAnnotation legendHeader = new ScreenAnnotation("jährliche Abholzungsrate \n \n -hohe Abholzung \n\n\n \n \n \n\n   -geringe Abholzung", new Point(920, 270));
        ...
        legendHeader.getAttributes().setImageSource(colorSchemePic.getPowerOfTwoImage());
...
//        annotationLayer.addAnnotation(ifgiLogo);
        ...
        ScreenAnnotation heightLegend = new ScreenAnnotation("Höhe entspricht dem Bruttoinlands- produkt pro Person", new Point(920,65));
...
//        generalAnnotationLayer.addAnnotation(gdpAnnotation);
        ...
        if(year == "2002"){
        	...
        	statAnnotation.setText("Pará \n Bevölkerung: " +f2.format((int)population) + "\n Jährliche Abholzung: "+f.format(averageDefor*100) +"%\nJährlich abgeholzte Fläche: "+f.format(averageDefor*totalArea) +"km²\n Bruttoinlandsprodukt: "+f.format(averageGDP*0.400125419)+" R$ ");
        }
        if(year == "2003"){
        ...
        if(year == "2004"){
        	factAnnotation.setText("Die jährliche Abholzung von "+f.format(averageDefor*totalArea)+ "km² entspricht " +f.format(averageDefor*totalArea/2569.69) +" mal der Fläche des Saarlandes.");
        	statAnnotation.setText("Pará \n Bevölkerung: " +f2.format((int)population) + "\n Jährliche Abholzung: "+f.format(averageDefor*100) +"%\nJährlich abgeholzte Fläche: "+f.format(averageDefor*totalArea) +"km²\n Bruttoinlandsprodukt: "+f.format(averageGDP*0.400125419)+" R$ ");
        ...
        if(year == "2008"){
        	factAnnotation.setText("Brasilien nutzt 16000 km² ("+(f.format(16000/2569.69))+"x Saarland) um Soja für Deutschlands Bedürfnisse anzubauen.");
        	statAnnotation.setText("Pará \n Bevölkerung: " +f2.format((int)population) + "\n Jährliche Abholzung: "+f.format(averageDefor*100) +"%\nJährlich abgeholzte Fläche: "+f.format(averageDefor*totalArea) +"km²\n Bruttoinlandsprodukt: "+f.format(averageGDP*0.400125419)+" R$ ");

        }
        
//        ScreenAnnotation backgroundAnnotation = new ScreenAnnotation("Fakten Ã¼ber Para", new Point(165, 80));
//        backgroundAnnotation.getAttributes().setDefaults(defaultAttributes2);
//        backgroundAnnotation.getAttributes().setBackgroundColor(new Color(0.2f, 0.2f, 0.2f, .5f));
//      // backgroundAnnotation.getAttributes().setOpacity(0.5);
...
//        
////        ScreenAnnotation factAnnotation = new ScreenAnnotation("Fakten Ã¼ber Para:", new Point(30, 425));
////        factAnnotation.getAttributes().setDefaults(defaultAttributes2);
////        factAnnotation.getAttributes().setCornerRadius(0);
//////        factAnnotation.getAttributes().setSize(new Dimension(50, 0));
////        factAnnotation.getAttributes().setAdjustWidthToText(AVKey.SIZE_FIXED); // use strict dimension width - 200
////        factAnnotation.getAttributes().setDrawOffset(new Point(100, 0)); // screen point is annotation bottom left corner
////     //   factAnnotation.getAttributes().setHighlightScale(1);             // No highlighting either
...
////        if(year=="2006"){
////        	ScreenAnnotation statement2008Annotation = new ScreenAnnotation("\"Brasiliens Fleisch Exporte steigen um 14% Jährlich\"", new Point(165, 380));
...
////            statement2008Annotation.getAttributes().setAdjustWidthToText(AVKey.SIZE_FIT_TEXT); // use strict dimension width - 200
////
...
////        }
//        
//        ScreenAnnotation statAnnotation = new ScreenAnnotation("", new Point(175, 30));
//        statAnnotation.getAttributes().setDefaults(defaultAttributes2);
//        statAnnotation.getAttributes().setBackgroundColor(new Color(0.2f, 0.2f, 0.2f, .5f));
//      // backgroundAnnotation.getAttributes().setOpacity(0.5);
//        statAnnotation.getAttributes().setBorderColor(Color.BLACK);
